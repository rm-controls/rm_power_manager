...
void GUI_InitChart(Chart_Struct_t *Chart) {
    ...
    GUI_DrawRectangle(Chart->X_Pos, Chart->Y_Pos, Chart->Width, Chart->Height, C_WHITE, Filled);
    if (Chart->Coordinate_Style == Rectangular_Coordinate) {
        for (unsigned char counter = 0; counter < 6; counter++)
            GUI_Point(Chart->X_Pos + counter / 2, Chart->Y_Pos + counter, C_BLACK);
        for (unsigned char counter = 0; counter < 6; counter++)
            GUI_Point(Chart->X_Pos + Chart->Width - counter,
                      Chart->Y_Pos + Chart->Height - counter / 2,
                      C_BLACK);
        GUI_DrawHLine(Chart->X_Pos, Chart->Y_Pos + Chart->Height, Chart->Width, C_BLACK);
        GUI_DrawVLine(Chart->X_Pos, Chart->Y_Pos, Chart->Height, C_BLACK);
    }
}
...
void GUI_CurveAppend(Curve_Struct_t *Curve, float Value) {
    ...
    if (Curve->Pos_Counter < (Curve->Chart->Width - 3))
        ...
        if (Curve->Chart->Coordinate_Style == Rectangular_Coordinate) {
            for (unsigned char counter = 0; counter < 6; counter++)
                GUI_Point(Curve->Chart->X_Pos + counter / 2, Curve->Chart->Y_Pos + counter, C_BLACK);
            for (unsigned char counter = 0; counter < 6; counter++)
                GUI_Point(Curve->Chart->X_Pos + Curve->Chart->Width - counter,
                          Curve->Chart->Y_Pos + Curve->Chart->Height - counter / 2,
                          C_BLACK);
            GUI_DrawHLine(Curve->Chart->X_Pos,
                          Curve->Chart->Y_Pos + Curve->Chart->Height,
                          Curve->Chart->Width,
                          C_BLACK);
            GUI_DrawVLine(Curve->Chart->X_Pos, Curve->Chart->Y_Pos, Curve->Chart->Height, C_BLACK);
        }
        Curve->Amplification_Factor = 1000.0f;
