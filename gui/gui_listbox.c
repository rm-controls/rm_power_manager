...
void GUI_InitListBox(ListBox_Struct_t *ListBox) {
    ...
    gui_draw_rectangle(ListBox->X_Pos, ListBox->Y_Pos, ListBox->Width, ListBox->Height, C_WHITE, Filled);
    gui_draw_rectangle(ListBox->X_Pos, ListBox->Y_Pos, ListBox->Width, ListBox->Height, LISTBOX_COLOR_EDGE, UnFilled);
}
...
void GUI_ListBoxDisplayItem(ListBox_Struct_t *ListBox,
    ...
    if (InvertColor == 0) {
        while (str[n] != '\0') {
            gui_putchar(row, column, str[n], C_BLACK, C_WHITE);
            row += 6;
            n++;
        }
        ...
        gui_draw_rectangle(row, column, (20 - n) * 6 - 4, 12, LISTBOX_COLOR_FOCUS, Filled);
    }
}
